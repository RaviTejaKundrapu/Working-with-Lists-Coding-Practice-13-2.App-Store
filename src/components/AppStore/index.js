...
class AppStore extends Component {
  state = {
    searchInput: '',
    appsList: initialAppList,
    seeByTabId: tabsList[0].tabId,
  }
  ...
  onChangeItemsByTab = tabId => {
    this.setState({seeByTabId: tabId}, this.getFilterProjectsByTab)
  }

  getFilterProjectsByTab = () => {
    const {appsList, seeByTabId} = this.state
    const filteredByTab = appsList.filter(
      appByTab => appByTab.category === seeByTabId,
    )
    return filteredByTab
  }

  render() {
    const filteredAppsList = this.getFilterProjectsByTab()
    const {searchInput, seeByTabId} = this.state
    const searchResult = filteredAppsList.filter(eachApp =>
        ...
        <ul className="tabsList">
          {tabsList.map(tab => (
            <TabItem
              tabsList={tab}
              key={tab.tabId}
              onChangeItemsByTab={this.onChangeItemsByTab}
              isAccessing={tab.tabId === seeByTabId}
            />
          ))}
        </ul>
        <ul className="appsList">
          {searchResult.map(app => (
            <AppItem appsList={app} key={app.appId} />
          ))}
        </ul>
      </div>
    )
